...
- [ ] Установка Meta XR SDK (OpenXR backend) через Package Manager
  - Scoped registry Meta XR (`https://npm.developer.meta.com`, scope `com.meta.xr`) уже настроен в `Packages/manifest.json`
- [ ] XR Plug-in Management → OpenXR (+ Meta Quest feature group)
- [ ] Проверка консоли Unity на ошибки компиляции

### 2.2. Сцена
- [ ] Сцена `Assets/Scenes/BleTextScene`: XR Origin (Meta XR), Directional Light, EventSystem
- [ ] `OVRPassthroughLayer` (Colour) на XR Origin (FR-4)
- [ ] `BleTextRoot`: TextMeshPro 3D + подложка, позиция 1.5 м перед камерой, billboard
- [ ] Статус-лейбл подключения (TMP: Scanning… / Connected / Reconnecting)

### 2.3. Нативный BLE-мост (JNI)
- [ ] Java-класс `BleBridge` (`Assets/Plugins/Android/`): scan по имени `ESP32-BLE-Text`, connect, discoverServices, subscribe Notify (descriptor 0x2902), колбэки в main thread
- [ ] Сборка в `.jar`/`.aar` (или java-плагин через Gradle)
- [ ] C# `BleClientService.cs`: обёртка через `AndroidJavaObject`, события `OnTextReceived(string)`, `OnStateChanged`, диспетчеризация в Unity main thread

### 2.4. C#-скрипты `Assets/Scripts/Ble/`
- [ ] `BleClientService.cs` (см. 2.3)
- [ ] `BleReconnector.cs`: таймаут сканирования, перезапуск, backoff, автопереподключение (FR-5)
- [ ] `TextDisplayController.cs`: подписка на `OnTextReceived`, обновление TMP, скрытие при `""`/`CLR` (FR-6)
- [ ] `HeadLockedText.cs`: размещение 1.5 м от камеры (FR-4)
- [ ] Проверка компиляции (read_console: без ошибок)

### 2.5. Настройки Android
- [ ] Кастомный `AndroidManifest.xml` (`Assets/Plugins/Android/`): `BLUETOOTH_SCAN` (neverForLocation), `BLUETOOTH_CONNECT`, `CAMERA`
- [ ] Player Settings: Min API 29 (сейчас 25), IL2CPP + ARM64, package name
- [ ] URP-настройки под Quest (рекомендации Meta)
